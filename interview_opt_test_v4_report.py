...
# 고정 파일 경로
CAND_CSV = Path("candidate_activities_input_before_test_v4_SISUL.csv")
GRID_CSV = Path("parameter_grid_test_v4.csv")
YAML_FILE = Path("precedence_config_test_v4_SISUL.yaml")

OUT_WIDE = Path("schedule_wide_test_v4_SISUL.csv")
OUT_LOG  = Path("log/run_log_test_v4_SISUL.csv")
...
def main():
    ...
    cfg = {
        "df_raw"       : df_raw,                              # ← 재활용
        "cfg_duration" : pd.read_csv("duration_config_test_v4_SISUL.csv"),
        "cfg_avail"    : pd.read_csv("space_availability_test_v4_SISUL.csv",
                                     parse_dates=["date"]),
        "cfg_map"      : pd.read_csv("activity_space_map_test_v4_SISUL.csv"),
        "cfg_oper"     : pd.read_csv("operating_config_test_v4_SISUL.csv",
                                     parse_dates=["date"]),
    }
...
CSV  = Path('schedule_wide_test_v4_SISUL.csv')     # ← 입력
XLSX = Path('schedule_view_test_v4_SISUL.xlsx')    # ← 결과
