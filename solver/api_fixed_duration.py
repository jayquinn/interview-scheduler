...
def schedule_interviews(
    ...
    try:
        ...
        global_config_obj = GlobalConfig(
            ...
            time_settings={act: data["duration"] for act, data in activities.items()},
...
def _apply_smart_integration(cfg_ui: dict, logs_buffer: List[str]) -> dict:
    ...
    for pred, succ in adjacent_pairs:
        ...
        integrated_duration = pred_info["duration"] + succ_info["duration"]
        ...
        # 방 설정 최적화 (선행 활동 전용 방 제거) - UI 설정 보호
        ...
        if pred_room_type != integrated_room_type:
            ...
            # UI에서 설정한 방은 제거하지 않도록 보호
            ui_room_types = set()
            if "room_template" in cfg_ui:
                ui_room_types = set(cfg_ui["room_template"].keys())
            
            # UI에서 설정하지 않은 방만 제거
            if pred_room_type not in ui_room_types:
                if pred_count_col in room_plan_df.columns:
                    logs_buffer.append(f"  방 설정 최적화: {pred_room_type} 제거 (UI 설정 아님)")
                ...
                logs_buffer.append(f"  방 설정 보호: {pred_room_type} 유지 (UI 설정)")
...
def _convert_ui_data(
    ...
    for _, row in activities_df.iterrows():
        ...
        activities[row["activity"]] = {
            "mode": row["mode"],
            "duration": int(row.get("duration", row.get("duration", 20))),
            "room_type": row["room_type"],
            "min_capacity": int(row.get("group_size", row.get("min_cap", 1))),
            "max_capacity": int(row.get("max_group_size", row.get("max_cap", 1)))
        }
    ...
    # UI에서 설정한 방 템플릿 우선 사용
    room_template = cfg_ui.get("room_template", {})
    if room_template:
        for room_type, room_data in room_template.items():
            rooms[room_type] = {
                "count": room_data.get("count", 1),
                "capacity": room_data.get("cap", 1)
            }
        logs_buffer.append(f"UI 방 템플릿에서 {len(rooms)}개 방 타입 로드: {list(rooms.keys())}")
    
    # room_plan에서 추가 방 정보 보완
    if not room_plan_df.empty:
        for room_type in activities_df["room_type"].dropna().unique():
            if room_type not in rooms:  # UI 템플릿에 없는 경우만
    ...
    for date, date_result in result.results.items():
        ...
        for item in date_result.schedule:
            ...
            schedule_data.append({
                ...
                "duration": int(round((item.end_time - item.start_time).total_seconds() / 60 / 5) * 5)
            })
