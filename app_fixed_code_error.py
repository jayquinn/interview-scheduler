...
if not acts_df.empty:
    # 컬럼 매핑 수정 (UI → 스케줄러)
    if "duration_min" in acts_df.columns and "duration" not in acts_df.columns:
        acts_df = acts_df.rename(columns={"duration_min": "duration"})
    elif "duration" not in acts_df.columns and "duration_min" not in acts_df.columns:
