        ...
        if room_template:
            for room_type, config in room_template.items():
                if isinstance(config, dict):
                    # cap → capacity 매핑
                    if "cap" in config and "capacity" not in config:
                        config["capacity"] = config.pop("cap")
                    # count가 없으면 기본값 추가
    ...
    if "room_plan" not in st.session_state:
        ...
        if room_template:
            ...
            for rt, values in room_template.items():
                ...
                # cap 또는 capacity 키 사용
                if 'cap' in values:
                    final_plan_dict[f"{rt}_cap"] = values['cap']
                elif 'capacity' in values:
                    final_plan_dict[f"{rt}_cap"] = values['capacity']
                else:
                    final_plan_dict[f"{rt}_cap"] = 6  # 기본값
    ...
    for _, row in df.iterrows():
        ...
        if '+' in activity_name:
            ...
            if len(parts) == 2:
                ...
                prep_row['duration'] = _get_activity_duration(pred_activity, default=5)
                ...
                # 방 정보는 원본 유지 (발표면접실)
                interview_row['duration'] = _get_activity_duration(succ_activity, default=15)
...
def _adjust_stage_times(prep_row, interview_row):
    ...
    # 발표준비: 원본 시간부터 5분
    prep_duration = prep_row.get('duration', 5)
    ...
    if end_col:
        ...
        interview_duration = interview_row.get('duration', 15)
...
if not acts_df.empty:
    # 컬럼 매핑 수정 (UI → 스케줄러)
    if "duration" in acts_df.columns and "duration" not in acts_df.columns:
        acts_df = acts_df.rename(columns={"duration": "duration"})
    elif "duration" not in acts_df.columns and "duration" not in acts_df.columns:
        # duration 컬럼이 없으면 기본값 추가
        acts_df["duration"] = 10
    if "group_size" in acts_df.columns and "min_cap" not in acts_df.columns:
        acts_df = acts_df.rename(columns={"group_size": "min_cap"})
    
    if "use" in acts_df.columns:
        # use=True인 활동만 필터링
...
job_df = st.session_state.get("job_acts_map", pd.DataFrame())
if not job_df.empty:
    # count 컬럼이 없으면 대체 컬럼 찾기
    if "count" not in job_df.columns:
        possible_count_columns = ["인원수", "applicant_count", "total_count", "number"]
        for col in possible_count_columns:
            if col in job_df.columns:
                job_df = job_df.rename(columns={col: "count"})
                st.session_state["job_acts_map"] = job_df  # 수정된 데이터로 업데이트
                break
        else:
            # count 컬럼이 없으면 기본값 추가
            job_df["count"] = 1
            st.session_state["job_acts_map"] = job_df  # 수정된 데이터로 업데이트
...
# code 컬럼이 있는지 확인 후 중복 검사
if "code" in job_df.columns and job_df["code"].duplicated().any():
            ...
            with tab:
                ...
                for col in day_schedule.columns:
                    if col not in display_cols and (
                        ...
                        col in ['activity_name', 'room_name', 'duration', 'group_number', 'group_size']
                    ):
...
for col, hdr in [("duration", "소요시간(분)"), ("min_cap", "최소 인원"), ("max_cap", "최대 인원")]:
    gb.configure_column(
        col,
        header_name=hdr,
        editable=True,
