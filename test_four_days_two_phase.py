#!/usr/bin/env python3
"""
4일치 데이터 2단계 하드 제약 스케줄링 테스트
app.py의 기본 4일치 데이터로 2단계 스케줄링 테스트
"""
...
def test_four_days_two_phase_scheduling():
    """4일치 데이터 2단계 하드 제약 스케줄링 테스트"""
    
    print("[4일치 데이터 2단계 하드 제약 스케줄링 테스트]")
    print("=" * 60)
    ...
    params = {
        "min_gap_min": 5,
        "time_limit_sec": 120,  # 4일치이므로 시간 증가
    ...
    if status == "SUCCESS":
        ...
        if final_df is not None and not final_df.empty:
            output_file = f"four_days_two_phase_test_result_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
if __name__ == "__main__":
    test_four_days_two_phase_scheduling() 